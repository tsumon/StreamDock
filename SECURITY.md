...
- 登录 / 首次 setup：同一客户端 1 分钟内 5 次失败后锁定 60 秒，并返回 `Retry-After`。客户端默认按直连 TCP peer 识别；只有 peer 命中 `TRUSTED_PROXY_CIDRS` 时才采纳 `X-Real-IP` 或 `X-Forwarded-For` 的第一个合法 IP。默认空 = 忽略转发头
